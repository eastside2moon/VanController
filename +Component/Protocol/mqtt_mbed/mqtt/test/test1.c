...
struct Options
{
    char *ion;         /**< ion to system under test. */
    char **haions;
    int hacount;
    int verbose;
    int test_no;
    ...
    while ( count < argc )
        {
            if ( strcmp ( argv[count], "--test_no" ) == 0 )
                ...
                if ( strcmp ( argv[count], "--ion" ) == 0 )
                    {
                        if ( ++count < argc )
                            {
                                options.ion = argv[count];
                                printf ( "\nSetting ion to %s\n", options.ion );
                            }
                    ...
                    if ( strcmp ( argv[count], "--haions" ) == 0 )
                        {
                            if ( ++count < argc )
                                {
                                    char *tok = strtok ( argv[count], " " );
                                    options.hacount = 0;
                                    options.haions = malloc ( sizeof ( char * ) * 5 );

                                    while ( tok )
                                        {
                                            options.haions[options.hacount] = malloc ( strlen ( tok ) + 1 );
                                            strcpy ( options.haions[options.hacount], tok );
                                            options.hacount++;
                                            tok = strtok ( NULL, " " );
                                        }
                                }
...
int checkPackets ( MQTTPacket_Data *before, MQTTPacket_Data *after )
{
    int rc = 0;
...
{
    MQTTPacket_Data data = MQTTPacket_Data_initializer;
    MQTTPacket_Data data_after = MQTTPacket_Data_initializer;
    int rc = 0;
    char buf[100];
    ...
    failures = 0;
    MyLog ( LOGA_INFO, "Starting test 1 - serialization of  and back" );
    ...
    rc = MQTTSerialize_ ( buf, buflen, &data );
    assert ( "good rc from serialize ", rc > 0, "rc was %d\n", rc );
    rc = MQTTDeserialize_ ( &data_after, buf, buflen );
    assert ( "good rc from deserialize ", rc == 1, "rc was %d\n", rc );
    /* data after should be the same as data before */
    rc = checkPackets ( &data, &data_after );
